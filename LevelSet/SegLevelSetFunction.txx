//
//  SegLevelSetFunction.cc
//  
//  22-dec-11  bhb  From ItkSnap SNAPLevelSetFunction.cxx
//  Modified:
//	06-jul-17  bhb	reformated to my taste
//	
...
SegLevelSetFunction<TImageType>::SegLevelSetFunction() : Superclass()
{
...
template<class TImageType>
SegLevelSetFunction<TImageType>
::~SegLevelSetFunction()
{
...
template<class TImageType>
void 
SegLevelSetFunction<TImageType>::SetSpeedImage(ImageType *pointer)
{
...
template<class TImageType>
void SegLevelSetFunction<TImageType>::CalculateInternalImages()
{
	...
	if(m_LaplacianSmoothingSpeedExponent != 0)
		m_LaplacianSmoothingSpeedInterpolator->SetInputImage( m_LaplacianSmoothingSpeedImage);
...
template<class TImageType>
typename SegLevelSetFunction<TImageType>::ScalarValueType
SegLevelSetFunction<TImageType>::CurvatureSpeed(const NeighborhoodType &neighborhood, 
...
template<class TImageType>
typename SegLevelSetFunction<TImageType>::ScalarValueType
SegLevelSetFunction<TImageType>::PropagationSpeed(const NeighborhoodType &neighborhood, 
					const FloatOffsetType &offset, GlobalDataStruct *) const 
{
	// If the exponent is zero, there is nothing to return
	if(m_PropagationSpeedExponent == 0)
...
template<class TImageType>
typename SegLevelSetFunction<TImageType>::ScalarValueType
SegLevelSetFunction<TImageType>::LaplacianSmoothingSpeed(const NeighborhoodType &neighborhood, 
						const FloatOffsetType &offset, GlobalDataStruct *) const 
{
	// If the exponent is zero, there is nothing to return
	if(m_LaplacianSmoothingSpeedExponent == 0)
	...
	if ( m_LaplacianSmoothingSpeedInterpolator->IsInsideBuffer(cdx) )
	{
		...
		return ( static_cast<ScalarValueType>(m_LaplacianSmoothingSpeedImage->GetPixel(idx)) );
}

template <class TImageType>
typename SegLevelSetFunction<TImageType>::VectorType
SegLevelSetFunction<TImageType>::AdvectionField(const NeighborhoodType &neighborhood,
				 const FloatOffsetType &offset, GlobalDataStruct *) const
{
	IndexType idx = neighborhood.GetIndex();
	...
	if ( m_AdvectionFieldInterpolator->IsInsideBuffer(cdx) )
		return ( m_VectorCast(m_AdvectionFieldInterpolator->EvaluateAtContinuousIndex(cdx)));
	else
		return ( m_AdvectionField->GetPixel(idx) );
}

template <class TImageType>
void
SegLevelSetFunction<TImageType>::PrintSelf(std::ostream &os, itk::Indent indent) const
{
